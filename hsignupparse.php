...
$fname = $lname = $email = $phone = $dob = $gender = $dept = $type = $staffid = $pass = $cpass = $tokenkey = $token = " ";
...
/***********************************************/
// Data Formatting
$fnError = $lnError = $emailError = $phoneError = $idError = $passError = $cpassError = "";
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    if (empty($_POST["rollno"])) {
        ...
        $staffid = test_input($_POST["rollno"]);
    }
}
...
/***********************************************/
$query1 = "SELECT * FROM host_table WHERE email= '$email' and verified = 1";
...
if ($rows == 1) {
    ...
    if (isset($fname) && ($lname) && ($email) && ($phone) && ($dob) && ($gender) && ($dept) && ($type) && ($staffid) && ($pass)) {
        ...
        if ($cpass == $pass) {
            ...
            if (filter_var($email, FILTER_VALIDATE_EMAIL)) {
                ...
                if (strlen($pass) >= 8 && strlen($cpass) <= 20) {
                    ...
                    // Add data
                    $query = "INSERT into `host_table` (fName, lName, email, phone, staffid, dept, stafftype, dob, gender, password, verified, vkey)
                					VALUES ('$fname', '$lname', '$email', '$phone', '$staffid', '$dept', '$type', '$dob', '$gender', '$password', '$verifystatus', '$token')";
