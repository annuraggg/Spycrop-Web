...
if (isset($_POST['email'])) {
  ...
  } else if (isset($_POST['hostSubmit'])) {
    $email = $_POST['email'];
    $pass1 = $_POST['password'];

    $query    = "SELECT * FROM `host_table` WHERE email='$email'";
    $result = mysqli_query($con, $query);
    $rows = mysqli_num_rows($result);
      ...
      <form action="" method="post">
        <div class="row">
          ...
          <div class="col-lg-5 form__buttons">
            <button type="submit" name="userSubmit" value="user" />
            <i class="fa-solid fa-user"></i>LOG IN AS USER
            </button>
            <button type="submit" name="hostSubmit" value="host">
              <i class="fa-solid fa-chalkboard-user"></i>LOG IN AS HOST
            </button>
          </div>
        </div>
      </form>
