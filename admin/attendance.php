...
if (isset($_GET['searchquery'])) {
   ...
   $query    = "SELECT * FROM `host_attendance` WHERE staffid = $searchquery";
   $result = mysqli_query($con, $query);
} else {
   $query    = "SELECT * FROM `host_attendance`";
   $result = mysqli_query($con, $query);
}
?>
               ...
               <table>
                  ...
                  if ($result->num_rows >= 0) {
                     while ($row = $result->fetch_assoc()) {
                        echo "<tr><td>" . $row["userid"] . "</td><td>" . $row["staffid"] . "</td><td>" . $row["datestamp"] . "</td><td>" . $row["timestamp"] . "</td><td>" . $row["status"] . "</td>"  . "</tr>";
                     }
                  }
                  ?>
               </table>
